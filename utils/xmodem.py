...
class XMODEM(object):
    ...
    def send(self, stream, retry=16, timeout=60, quiet=False, callback=None):
        ...
        #print('xxx Begin start sequence, packet_size=%d'%packet_size)
        error_count = 0
        crc_mode = 0
        cancel = 0
        while True:
            ...
            if char:
                if char == NAK:
                   ...
                   print('error:','send error: expected NAK, CRC, EOT or CAN; got %r'%char)
        ...
        while True:
            ...
            while True:
                ...
                print('error:','send error: expected ACK; got %r for block %d', char, sequence)
                error_count += 1
                if callable(callback):
                    callback(total_packets, success_count, error_count)
                if error_count > retry:
                    ...
                    print('error:','send error: NAK received %d times, aborting.', error_count)
                    self.abort(timeout=timeout)
                    return False
        ...
        while True:
            ...
            if char == ACK:
                ...
                print('error:','send error: expected ACK; got %r', char)
                error_count += 1
                if error_count > retry:
                    #self.log.warn('EOT was not ACKd, aborting transfer')
                    print('warn:','warn:EOT was not ACKd, aborting transfer')
                    self.abort(timeout=timeout)
                    return False
    ...
    def sendb(self, bdata, retry=16, timeout=60, quiet=False, callback=None):
        '''
        Send a bdata via the XMODEM protocol.

            >>> bdata = open('/etc/issue', 'rb')
            >>> print(modem.send(bdata))
            True

        Returns ``True`` upon successful transmission or ``False`` in case of
        failure.

        :param bdata: The bdata object to send data from.
        :type bdata: bdata (file, etc.)
        :param retry: The maximum number of times to try to resend a failed
                      packet before failing.
        :type retry: int
        :param timeout: The number of seconds to wait for a response before
        ...
        sequence = 1
        while True:
            data = bdata[:packet_size]
            bdata = bdata[packet_size:]
            if not data:
                # end of bdata
                #self.log.debug('send: at EOF')
                break
        ...
        while True:
            ...
            if char == ACK:
                ...
                if error_count > retry:
                    #self.log.warn('EOT was not ACKd, aborting transfer')
                    print('warn:EOT was not ACKd, aborting transfer')
                    self.abort(timeout=timeout)
                    return False
    ...
    def recv(self, stream, crc_mode=1, retry=16, timeout=60, delay=1, quiet=0):
        ...
        while True:
            ...
            if char is None:
                ...
                print('warn:recv error: getc timeout in start sequence')
                error_count += 1
                continue
            elif char == SOH:
        ...
        while True:
            ...
            if seq1 is None:
                ...
                print('warn: getc failed to get first sequence byte')
                seq2 = None
            else:
                seq1 = ord(seq1)
                ...
                if seq2 is None:
                    ...
                    print('warn:getc failed to get second sequence byte')
                else:
            ...
            if not (seq1 == seq2 == sequence):
                ...
                print('error:','expected sequence %d, got (seq1=%r, seq2=%r), receiving next block, will NAK.', sequence, seq1, seq2)
            ...
            print('warn:','recv error: purge, requesting retransmission (NAK)')
            while True:
    ...
    def _verify_recv_checksum(self, crc_mode, data):
        if crc_mode:
            ...
            if not valid:
                print('warn:','recv error: checksum fail (theirs=%04x, ours=%04x), '%(their_sum, our_sum))
            ...
            if not valid:
                print('warn:','recv error: checksum fail (theirs=%02x, ours=%02x)'%(their_sum, our_sum))
